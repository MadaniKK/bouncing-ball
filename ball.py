...
class Ball:
    ...
    def move(self):
        ...
        if self.position[0] <= 0 + self.radius or self.position[0] >= self.screen.get_width() - self.radius:
            self.velocity[0] *= -1
        if self.position[1] <= 0 + self.radius or self .position[1] >= self.screen.get_height() - self.radius:
            self.velocity[1] *= -1
